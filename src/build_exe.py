...
def build_version_components(now: datetime) -> tuple[int, int, int, int, str]:
    """Erzeuge Windows-kompatible 4-teilige Build-Version + Textdarstellung.

    Windows-FileVersion benötigt vier 16-bit-Integer (0..65535).
    """
    build = (now.year % 100) * 1000 + now.timetuple().tm_yday
    revision = (now.hour * 3600 + now.minute * 60 + now.second) // 2
    version_text = f"{VERSION_MAJOR}.{VERSION_MINOR}.{build}.{revision}"
    return VERSION_MAJOR, VERSION_MINOR, build, revision, version_text
...
def build_exe():
    ...
    now = datetime.now()
    major, minor, build, revision, version_text = build_version_components(now)
    ...
    write_windows_version_file(version_file, (major, minor, build, revision), version_text, build_stamp)
